...
import { suggestSavingsOpportunities } from "@/lib/livePlan";
...
export default function TimelinePage() {
  ...
  return (
    <>
      ...
      {settings.savingsCoach.nudgesEnabled && biggestGap && (
        <GoalProtectionNudges
          shortfall={biggestGap.shortfall}
          onTrackCount={onTrackCount}
          totalGoals={decorated.length}
          opportunities={suggestSavingsOpportunities(finances)
            .filter((o) => !savingsGame.appliedOpportunityIds.includes(o.id))
            .slice(0, 2)}
          onApplyRecovered={(opportunityId, monthlyRecoverable) => {
            const applied = applyGoalProtectionNudge(opportunityId);
            if (!applied) return;
            trackSavingsCoachInteraction("accepted", monthlyRecoverable);
            setMonthlySIP((v) => Math.max(v, 0) + monthlyRecoverable);
          }}
        />
      )}
...
function GoalProtectionNudges({
  ...
  onTrackCount: number;
  totalGoals: number;
  opportunities: { id: string; title: string; tip: string; monthlyRecoverable: number }[];
  onApplyRecovered: (opportunityId: string, monthlyRecoverable: number) => void;
}) {
  return (
    <div className="mb-4 rounded-2xl border border-[var(--color-edge)] bg-[var(--color-panel)] p-4">
      <div className="text-sm font-semibold">Goal protection nudges</div>
      <div className="text-xs text-[var(--color-ink-dim)] mt-0.5">
        {onTrackCount} of {totalGoals} goals are on track. Closing even part of ₹{compact(shortfall)} shortfall now compounds faster.
      </div>
      <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
        {opportunities.map((o) => (
          <div key={o.id} className="rounded-xl bg-[var(--color-grid)] px-3 py-3">
            <div className="text-xs font-medium">{o.title}</div>
            <div className="text-[11px] text-[var(--color-ink-dim)] mt-0.5">{o.tip}</div>
            <div className="mt-2 flex items-center justify-between gap-2">
              <span className="text-xs text-[var(--color-mint-dim)] font-semibold">
                Recover ₹{compact(o.monthlyRecoverable)}/mo
              </span>
              <button
                onClick={() => onApplyRecovered(o.id, o.monthlyRecoverable)}
                className="rounded-full bg-[var(--color-pill-dark)] text-white px-3 py-1.5 text-[11px] font-medium hover:opacity-90"
              >
                Apply
              </button>
            </div>

...
export default function SandboxPage() {
  ...
  const proj = useMemo(
    () =>
      buildProjection({
        ...
        // Exclude retirement milestones: retirement represents target corpus, not a lump-sum drawdown.
        // The persona's own projection similarly only includes pre-retirement milestones.
        milestoneDrawdowns: persona.milestones
          .filter((m) => m.category !== "retirement")
          .map((m) => ({
            age: m.age,
  ...
  const finalReal = proj[proj.length - 1].real;
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[2fr_1fr] gap-4">
        <TerminalPanel title="LIVE PROJECTION" subtitle="changes update the curve in real time" active scanline>
          ...
          <div className="mt-3 grid grid-cols-2 lg:grid-cols-5 gap-2 text-[11px]">
            ...
            <Stat label="Real (today's ₹)" value={formatINR(finalReal, { compact: true })} />

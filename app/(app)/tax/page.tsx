...
function recommendations(personaId: string): { title: string; body: string; tag: string }[] {
  const map: Record<string, { title: string; body: string; tag: string }[]> = {
    ...
    family: [
      { title: "Split 80C between partners", body: "Use ELSS + PPF mix to maximize household-level deductions without concentration.", tag: "80C" },
      { title: "Coordinate annual LTCG harvesting", body: "Book gains across family equity holdings up to annual exemption bands and reset cost basis.", tag: "LTCG" },
      { title: "Use goal buckets for tax location", body: "Keep near-term family goals in debt-efficient wrappers; route long-term goals through equity/NPS.", tag: "LOCATION" },
    ],
  };

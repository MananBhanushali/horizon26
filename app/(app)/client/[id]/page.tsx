...
export default function ClientPage() {
  ...
  const firstName = client ? client.name.split(" ")[0] : persona.name.split(" ")[0];
  ...
  return (
    <div className="flex flex-col gap-5">
      {/* Greeting */}
      <div>
        <div className="text-[var(--color-ink-dim)] flex items-center gap-2 mb-2">
           <Link href="/dashboard" className="hover:underline text-[13px]">← Back to Firm Dashboard</Link>
        </div>
        <h1 className="text-3xl font-semibold tracking-tight">
          Client: {firstName}
        </h1>
        <p className="text-sm text-[var(--color-ink)] mt-1">
          Viewing financial plan and projections for {client?.name ?? "this client"}.
        </p>
      </div>
      ...
      {settings.savingsCoach.nudgesEnabled && (
        <SavingsCoachHub
          finances={finances}
          livePlan={livePlan}
          gamificationEnabled={settings.savingsCoach.gamificationEnabled}
          savingsGame={savingsGame}
          autoEscalateEnabled={settings.savingsCoach.autoEscalateEnabled}
          autoEscalatePct={settings.savingsCoach.autoEscalatePct}
          onApplyOpportunity={(amount, opportunityId) => {
            const applied = applyGoalProtectionNudge(opportunityId);
            if (!applied) return;
            updateFinances({ monthlySavings: finances.monthlySavings + amount });
            logSavingsWin(Math.round(amount / 4));
            trackSavingsCoachInteraction("accepted", amount);
            setWeeklyChallengeTarget(
              Math.max(
                savingsGame.weeklyTarget,
                Math.round((finances.monthlySavings + amount) * 0.22),
              ),
            );
          }}
          onDismissOpportunity={() => trackSavingsCoachInteraction("dismissed")}
          onSnoozeOpportunity={() => trackSavingsCoachInteraction("snoozed")}
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-[var(--color-panel)] border border-[var(--color-edge)] p-5 rounded-2xl shadow-sm">
             <div className="flex justify-between items-center mb-2">
               ...
               <div className="text-[10px] uppercase font-bold text-[var(--color-warn-dim)] bg-[var(--color-warn-dim)]/10 px-2 py-1 rounded">Monte Carlo</div>
             </div>
             <div className="text-xs text-[var(--color-ink-dim)] mb-4">Simulating 100 random market paths over 10 years for {client?.name}'s current AUM.</div>
             <div className="h-48 w-full bg-white border border-[var(--color-edge)] rounded-xl relative overflow-hidden shadow-inner">
                <div className="absolute inset-0">
                  <MonteCarloChart baseAmount={finances.emergencyFund + 1000000} years={10} expectedReturn={livePlan.effectiveAnnualReturn / 100} volatility={0.18} paths={100} />
                </div>
             </div>
...
function SavingsCoachHub({
  ...
  savingsGame: ReturnType<typeof useApp>["savingsGame"];
  autoEscalateEnabled: boolean;
  autoEscalatePct: number;
  onApplyOpportunity: (monthlyAmount: number, opportunityId: string) => void;
  onDismissOpportunity: () => void;
  onSnoozeOpportunity: () => void;
  savingsMetrics: ReturnType<typeof useApp>["savingsMetrics"];
}) {
  const impact = estimateSpendImpact(finances, livePlan);
  const appliedOpportunityIds = new Set(savingsGame.appliedOpportunityIds);
  const opportunities = suggestSavingsOpportunities(finances).filter(
    (o) => !appliedOpportunityIds.has(o.id),
  );
  const challengeProgress =
    savingsGame.weeklyTarget > 0
      ? Math.min(100, Math.round((savingsGame.weeklySaved / savingsGame.weeklyTarget) * 100))
      : 0;
  const xpPoints =
    savingsMetrics.nudgeAccepted * 60 +
    savingsGame.currentStreak * 45 +
    Math.round(savingsMetrics.estimatedMonthlyUplift / 200);
  const level = Math.max(1, Math.floor(xpPoints / 250) + 1);
  const levelBaseXp = (level - 1) * 250;
  const nextLevelXp = level * 250;
  const levelProgress = Math.min(
    100,
    Math.round(((xpPoints - levelBaseXp) / Math.max(1, nextLevelXp - levelBaseXp)) * 100),
  );
  const league =
    level >= 10 ? "Legend investor" : level >= 6 ? "Gold saver" : level >= 3 ? "Silver saver" : "Starter saver";
  const questItems = [
    { id: "quest-weekly", label: "Hit weekly target", done: challengeProgress >= 100 },
    { id: "quest-opportunity", label: "Apply 2 opportunities", done: appliedOpportunityIds.size >= 2 },
    { id: "quest-streak", label: "Build 4-week streak", done: savingsGame.currentStreak >= 4 },
  ];
  const challengeTier = savingsGame.currentStreak >= 4 ? "boss" : savingsGame.currentStreak >= 2 ? "pro" : "starter";
  const powerUpAmount =
    challengeTier === "boss"
      ? Math.max(4000, Math.round(Math.max(finances.monthlySavings, 0) * 0.12))
      : challengeTier === "pro"
        ? Math.max(2500, Math.round(Math.max(finances.monthlySavings, 0) * 0.08))
        : Math.max(1500, Math.round(Math.max(finances.monthlySavings, 0) * 0.05));
  const powerUpId = `powerup-${challengeTier}`;

  return (
    <section className="grid grid-cols-1 xl:grid-cols-[1.2fr_1fr] gap-5">
      <div className="h-panel p-5">
        <div className="text-sm font-semibold tracking-tight">Spend impact and transparency</div>
        <div className="text-xs text-[var(--color-ink-dim)] mt-0.5">
          Before spending, see exactly how much extra SIP you need to keep goals on track.
        </div>
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <MiniStat label="Required monthly savings" value={`₹${compact(impact.requiredMonthlySavings)}`} />
          <MiniStat label="Extra needed this month" value={`₹${compact(impact.extraMonthlyNeeded)}`} tone="warn" />
          <MiniStat label="Estimated goal delay" value={`${impact.estimatedDelayMonths} mo`} tone="warn" />
        </div>
        <div className="mt-4 rounded-2xl bg-[var(--color-grid)] p-4">
          <div className="text-xs font-medium">Savings opportunities</div>
          <div className="mt-3 flex flex-col gap-3">
            {opportunities.map((o) => (
              <div
                key={o.id}
                className="rounded-xl border border-[var(--color-edge)] bg-[var(--color-panel)] px-3 py-3"
              >
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-sm font-medium">{o.title}</div>
                    <div className="text-[11px] text-[var(--color-ink-dim)] mt-0.5">{o.tip}</div>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-sm font-semibold text-[var(--color-mint-dim)]">
                      +₹{compact(o.monthlyRecoverable)}/mo
                    </div>
                    <button
                      onClick={() => onApplyOpportunity(o.monthlyRecoverable, o.id)}
                      className="mt-1 text-[11px] text-[var(--color-cyan-dim)] hover:underline"
                    >
                      Apply to SIP
                    </button>
                    <div className="mt-1 flex items-center justify-end gap-2 text-[10px]">
                      <button
                        onClick={onSnoozeOpportunity}
                        className="text-[var(--color-ink-dim)] hover:underline"
                      >
                        Snooze
                      </button>
                      <button
        ...
        <div className="mt-3 text-[10px] text-[var(--color-ink-dim)]">
          Coach interactions: {savingsMetrics.nudgeAccepted} accepted · {savingsMetrics.nudgeSnoozed} snoozed · {savingsMetrics.nudgeDismissed} dismissed · +₹{compact(savingsMetrics.estimatedMonthlyUplift)}/mo estimated uplift
        </div>
      </div>

      <div className="h-panel p-5">
        <div className="text-sm font-semibold tracking-tight">Savings game loop</div>
        <div className="text-xs text-[var(--color-ink-dim)] mt-0.5">
          Complete quests, gain XP, and unlock stronger investment power-ups.
        </div>
        {gamificationEnabled ? (
          <>
            <div className="mt-4 rounded-2xl border border-[var(--color-edge)] bg-[var(--color-panel)] p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-xs text-[var(--color-ink-dim)]">Level {level}</div>
                  <div className="text-sm font-semibold">{league}</div>
                </div>
                <div className="text-right">
                  <div className="text-xs text-[var(--color-cyan-dim)] font-semibold">{xpPoints} XP</div>
                  <div className="text-[10px] text-[var(--color-ink-dim)]">
                    {nextLevelXp - xpPoints} XP to next level
                  </div>
                </div>
              </div>
              <div className="mt-2 h-2 rounded-full bg-[var(--color-grid)] overflow-hidden">
                <div
            ...
            <div className="mt-4 rounded-2xl bg-[var(--color-grid)] p-4">
              <div className="flex items-center justify-between text-xs">
                <span className="text-[var(--color-ink-dim)]">Weekly challenge</span>
                <span className="font-semibold">₹{compact(savingsGame.weeklyTarget)}</span>
              </div>
              <div className="mt-2 h-2 rounded-full bg-[var(--color-panel)] overflow-hidden">
                <div
                  className="h-full rounded-full bg-[var(--color-mint-dim)]"
                  style={{ width: `${challengeProgress}%` }}
                />
              </div>
              <div className="mt-2 text-[11px] text-[var(--color-ink-dim)]">
                Saved this week: ₹{compact(savingsGame.weeklySaved)} ({challengeProgress}%)
              </div>
            </div>
            <div className="mt-4 rounded-2xl bg-[var(--color-grid)] p-4">
              <div className="text-xs font-medium">Active quests</div>
              <div className="mt-2 flex flex-col gap-1.5">
                {questItems.map((q) => (
                  <div key={q.id} className="flex items-center justify-between text-[11px]">
                    <span className={q.done ? "text-[var(--color-mint-dim)]" : "text-[var(--color-ink-dim)]"}>
                      {q.done ? "✓" : "○"} {q.label}
                    </span>
                    {q.done && <span className="text-[10px] text-[var(--color-cyan-dim)]">+XP</span>}
                  </div>
                ))}
              </div>
            </div>
            <div className="mt-4 grid grid-cols-2 gap-3">
              <MiniStat label="Current streak" value={`${savingsGame.currentStreak} wk`} />
              <MiniStat label="Best streak" value={`${savingsGame.bestStreak} wk`} />
            </div>
            <div className="mt-4 rounded-2xl border border-[var(--color-edge)] bg-[var(--color-panel)] p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-xs text-[var(--color-ink-dim)]">Investment power-up</div>
                  <div className="text-sm font-semibold">
                    {challengeTier === "boss" ? "Boss boost" : challengeTier === "pro" ? "Pro boost" : "Starter boost"}
                  </div>
                </div>
                <button
                  onClick={() => onApplyOpportunity(powerUpAmount, powerUpId)}
                  disabled={appliedOpportunityIds.has(powerUpId)}
                  className="rounded-full bg-[var(--color-pill-dark)] text-white px-3 py-1.5 text-[11px] font-medium hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {appliedOpportunityIds.has(powerUpId)
                    ? "Used"
                    : `Boost SIP +₹${compact(powerUpAmount)}`}
                </button>
              </div>
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              {(savingsGame.badgesUnlocked.length > 0
                ? savingsGame.badgesUnlocked
                : ["First ₹10K saved", "4-week streak", "Goal Protector"]
              ).map((b) => (
                <AchievementBadge key={b} label={b} />
              ))}
            </div>
            {autoEscalateEnabled && (
              <div className="mt-4 text-[11px] text-[var(--color-ink-dim)]">
                Auto-escalate is on: recommend increasing monthly savings by {autoEscalatePct}% each quarter.
              </div>
            )}
          </>
        ) : (
          <div className="mt-4 text-xs text-[var(--color-ink-dim)]">
            Enable gamification in settings to track streaks and badges.
          </div>
        )}
      </div>
    </section>
...
function MiniStat({
  ...
  return (
    <div className="rounded-xl border border-[var(--color-edge)] bg-[var(--color-panel)] px-3 py-2.5">
      <div className="text-[10px] uppercase tracking-wider text-[var(--color-ink-dim)]">{label}</div>
      <div
        className="mt-1 text-base font-semibold tabular-nums"
        style={{ color: tone === "warn" ? "var(--color-warn-dim)" : "var(--color-ink)" }}
      >
        {value}
      </div>
    </div>

...
export function AssistantChatbot() {
  ...
  return (
    <div className="fixed bottom-5 right-4 z-50 sm:bottom-7 sm:right-7">
      {open && (
        <section className="mb-4 w-[min(94vw,420px)] rounded-2xl border-2 border-[var(--color-cyan-dim)] bg-[var(--color-panel)] text-[var(--color-ink)] shadow-xl">
          <header className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-edge)] bg-[var(--color-grid)]/60 rounded-t-2xl">
            ...
            <button
              ...
              className="grid h-7 w-7 place-items-center rounded-full text-[var(--color-ink)] hover:bg-[var(--color-grid)]"
          ...
          <div className="px-3.5 pb-2.5 flex flex-wrap gap-1.5">
            {QUICK_PROMPTS.map((prompt) => (
              <button
                ...
                className="rounded-full border border-[var(--color-edge)] px-2.5 py-1 text-[11px] font-medium text-[var(--color-ink)] hover:bg-[var(--color-grid)]"
              >
                {prompt}
              </button>
            ))}
          ...
          <form onSubmit={onSubmit} className="flex items-center gap-2 p-3.5 pt-2.5">
            <input
              ...
              className="h-10 flex-1 rounded-xl border border-[var(--color-edge)] bg-[var(--color-base)] px-3 text-sm text-[var(--color-ink)] placeholder:text-[var(--color-ink-dim)] outline-none focus:border-[var(--color-cyan-dim)]"
            />
            <button
              type="submit"
      ...
      <button
        ...
        className="group relative flex h-14 items-center gap-2.5 rounded-full border-2 border-[var(--color-cyan-dim)] bg-[var(--color-panel)] px-5 text-base font-semibold text-[var(--color-ink)] shadow-lg hover:bg-[var(--color-grid)]"
